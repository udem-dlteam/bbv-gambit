...
(define (type-fixnum-range-numeric tctx type)
  ...
  (let* ((fixnum-range (type-motley-fixnum-range type))
    ...
    (let ((lo (cond ((eq? lo '>=) (tctx-smallest-min-fixnum tctx))
                    ...
                    ((number? lo) lo)
                    (else         (tctx-smallest-min-fixnum tctx))))
           ...
           (if (not (eqv? 0 (bitwise-and
               ...
               (let ((length-range (type-motley-length-range type)))
                 (if (not (or (type-bot-fixnum-range? length-range)
                              (type-top-length-range? length-range)))
                     (let ((save result))
                       (set! result '())
...
(define (type-fixnum-always-< lohi1 lohi2) (eq? (type-fixnum-< lohi1 lohi2) #t))
(define (type-fixnum-always-> lohi1 lohi2) (eq? (type-fixnum-> lohi1 lohi2) #t))
(define (type-fixnum-always-<= lohi1 lohi2) (eq? (type-fixnum-<= lohi1 lohi2) #t))
(define (type-fixnum-always->= lohi1 lohi2) (eq? (type-fixnum->= lohi1 lohi2) #t))
(define (type-fixnum-never-< lohi1 lohi2) (eq? (type-fixnum-< lohi1 lohi2) #f))
(define (type-fixnum-never-> lohi1 lohi2) (eq? (type-fixnum-> lohi1 lohi2) #f))
(define (type-fixnum-never-<= lohi1 lohi2) (eq? (type-fixnum-<= lohi1 lohi2) #f))
(define (type-fixnum-never->= lohi1 lohi2) (eq? (type-fixnum->= lohi1 lohi2) #f))
...
(define (type-fixnum-hi-<-old hi1 hi2)
  (declare (generic))
  (cond ((eq? hi1 '<=)
         (if (length-bound? hi2)
...
(define (type-fixnum-min-hi-old x y)
  (case (type-fixnum-hi-<-old x y)
    ((#t) x)
    ((#f) y)
    (else
...
(define (type-fixnum-min-hi-old2 x y)
  (declare (generic))
  (cond ((eq? x '<=)
         y)
        ((eq? x '<)
        ...
        (else ;; (length-bound? x)
         (if (exact-integer? y)
             (if (< (length-bound-offset x) y)
                 x
                 y)
             (if (length-bound-same-object? x y)
                 (if (< (length-bound-offset x)
                        (length-bound-offset y))
                     x
                     y)
                 ;; TODO: consider replacing with (if (< (length-bound-offset x) (length-bound-offset y)) x y)
                 x)))))
...
(define (type-fixnum-min-hi x y)
  ...
  (cond ((eq? x '<=)
        ...
        ((eq? y '<)
         x)
        (else
         (if (< (if (exact-integer? x) x (length-bound-offset x))
                (if (exact-integer? y) y (length-bound-offset y)))
             x
             y))))

(define (type-fixnum-lo-<-old lo1 lo2)
  (declare (generic))
  (cond ((eq? lo1 '>=)
         (not (eq? lo2 '>=)))
        ((eq? lo1 '>)
...
(define (type-fixnum-max-lo-old x y)
  (case (type-fixnum-lo-<-old x y)
    ((#t) y)
    ((#f) x)
    (else
     (if (and (length-bound? x) (length-bound? y))
...
(define (type-fixnum-max-lo-old2 x y)
  (declare (generic))
  (cond ((eq? x '>=)
         (if (eq? y '>=) x y))
        ((eq? x '>)
        ...
        ((exact-integer? x)
         (if (exact-integer? y)
             (if (> x y) x y)
             (if (> x (length-bound-offset y)) x y)))
        (else ;; (length-bound? x)
         (if (exact-integer? y)
             (if (> (length-bound-offset x) y)
                 x
                 y)
             (if (length-bound-same-object? x y) ;; TODO: consider replacing with #t
                 (if (> (length-bound-offset x)
                        (length-bound-offset y))
                     x
                     y)
...
;;(set! type-fixnum-min-hi type-fixnum-min-hi-old)(set! type-fixnum-max-lo type-fixnum-max-lo-old)
